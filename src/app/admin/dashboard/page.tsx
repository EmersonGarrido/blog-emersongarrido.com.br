...
export default function AdminDashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [analyticsRes, commentsRes, subscribersRes, draftsRes] = await Promise.all([
        fetch(`/api/admin/analytics?period=${period}`),
        fetch(`/api/admin/comments?status=${commentFilter}`),
        fetch('/api/admin/subscribers?status=active'),
        fetch('/api/admin/posts?status=draft&limit=1')
      ])
      ...
      setDraftCount(draftsData.pagination?.total || 0)
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-2 mb-6">
          ...
          >
            ...
            Posts
            {draftCount > 0 && (
              <span className="bg-yellow-500/20 text-yellow-400 text-xs px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                {draftCount}
              </span>
            )}
          </Link>
