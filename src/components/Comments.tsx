...
export default function Comments({ postSlug }: CommentsProps) {
  ...
  return (
    <div id="comments" className="mt-6 pt-4 border-t border-[var(--border-color)]">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {!showForm && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-tertiary)] hover:bg-[var(--bg-hover)] rounded-lg transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            {locale === 'en' ? 'Comment' : 'Comentar'}
          </button>
        )}
      </div>
      ...
      {/* Comment Form - Only shows when clicking button */}
      <AnimatePresence>
        {showForm && (
          <motion.div
          ...
          >
            <form onSubmit={handleSubmit} className="mb-4 space-y-2 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-[var(--text-muted)]">
                  {locale === 'en' ? 'New comment' : 'Novo comentário'}
                </span>
                <button
                  type="button"
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-medium text-xs">
                    {comment.author_name || (locale === 'en' ? 'Anonymous' : 'Anônimo')}
                  </span>
